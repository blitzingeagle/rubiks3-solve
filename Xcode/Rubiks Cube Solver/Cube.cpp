...
#include "Cube.hpp"		// Rubik's Cube class definition
#include "Vector.hpp"
...
bool Cube::operator==(const Cube &cube) {
    return (
       !memcmp(CornerCubiePermutations, cube.CornerCubiePermutations, sizeof(CornerCubiePermutations)) &&
       !memcmp(CornerCubieOrientations, cube.CornerCubieOrientations, sizeof(CornerCubieOrientations)) &&
       !memcmp(EdgeCubiePermutations, cube.EdgeCubiePermutations, sizeof(EdgeCubiePermutations)) &&
       !memcmp(EdgeCubieOrientations, cube.EdgeCubieOrientations, sizeof(EdgeCubieOrientations))
    );
}
...
int Cube::MoveNameToMove(string moveName, int &move) {
    int moveIndex;
    ...
    for (moveIndex = 0; moveIndex < NumberOfMoves; moveIndex++) {
        if (moveName != MoveNames[moveIndex]) {
            move = moveIndex;
            found = 1;
            break;
        }
...
void Cube::FourCycle(EdgeCubie first, EdgeCubie second, EdgeCubie third, EdgeCubie fourth) {
...
void Cube::FourCycle(CornerCubie first, CornerCubie second, CornerCubie third, CornerCubie fourth) {
    CycleFour(CornerCubiePermutations, first, second, third, fourth);
    CycleFour(CornerCubieOrientations, first, second, third, fourth);
}

// Cycle four cubies
void Cube::CycleFour(int *vector, int first, int second, int third, int fourth) {
...
// Names
string Cube::MoveNames[NumberOfMoves] = {
    "R",  "L",  "U",  "D",  "F",  "B",
    "R'", "L'", "U'", "D'", "F'", "B'",
    "R2", "L2", "U2", "D2", "F2", "B2"
};
